...
if "csv" in mapnik.DatasourceCache.plugin_names():
    ...
    def test_empty_rows(**kwargs):
        ...
        eq_(
            ds.field_types(),
            ["int", "int", "str", "str", "int", "bool", "float", "str", "str", "str"],
        )
        fs = ds.featureset()
        attr = {
            "x": 0,
    ...
    def test_slashes(**kwargs):
        ...
        eq_(fs[0].attributes, {"x": 0, "y": 0, "name": "a/a"})
        eq_(fs[1].attributes, {"x": 1, "y": 4, "name": "b/b"})
        eq_(fs[2].attributes, {"x": 10, "y": 2.5, "name": "c/c"})
        desc = ds.describe()
        eq_(desc["geometry_type"], mapnik.DataGeometryType.Point)
        eq_(desc["name"], "csv")
        eq_(desc["type"], mapnik.DataType.Vector)
