...
def draw_title(m, ctx, text, size=10, color=mapnik.Color("black")):
    ...
    ctx.select_font_face(
        "DejaVu Sans Book", cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_NORMAL
    )
...
if mapnik.has_pycairo():
    ...
    def test_passing_pycairo_context_svg():
        ...
        diff = abs(
            os.stat(expected_cairo_file).st_size - os.stat(test_cairo_file).st_size
        )
        msg = "diff in size (%s) between actual (%s) and expected(%s)" % (
            diff,
            test_cairo_file,
            "tests/python_tests/" + expected_cairo_file,
        )
    ...
    def test_passing_pycairo_context_png():
        ...
        diff = abs(
            os.stat(expected_cairo_file2).st_size - os.stat(reduced_color_image).st_size
        )
        msg = "diff in size (%s) between actual (%s) and expected(%s)" % (
            diff,
            reduced_color_image,
            "tests/python_tests/" + expected_cairo_file2,
        )
        eq_(diff < 500, True, msg)
    ...
    if "sqlite" in mapnik.DatasourceCache.plugin_names():
        ...
        def _pycairo_surface(type, sym):
            ...
            if hasattr(cairo, "%sSurface" % type.upper()):
                surface = getattr(cairo, "%sSurface" % type.upper())(
                    test_cairo_file, m.width, m.height
                )
