...
if "geojson" in mapnik.DatasourceCache.plugin_names():
    ...
    def test_geojson_properties():
        ...
        eq_(f["name"], "Test")
        eq_(f["int"], 1)
        eq_(f["description"], "Test: \u005C")
        eq_(f["spaces"], "this has spaces")
        eq_(f["double"], 1.1)
        eq_(f["boolean"], True)
    ...
    def test_large_geojson_properties():
        ds = mapnik.Datasource(
            type="geojson", file="../data/json/escaped.geojson", cache_features=False
        )
        f = list(ds.features_at_point(ds.envelope().center()))[0]
        eq_(len(ds.fields()), 11)
        desc = ds.describe()
    ...
    def test_parsing_feature_collection_with_top_level_properties():
        ds = mapnik.Datasource(
            type="geojson", file="../data/json/feature_collection_level_properties.json"
        )
        f = list(ds.all_features())[0]

        desc = ds.describe()
        eq_(desc["geometry_type"], mapnik.DataGeometryType.Point)
        eq_(f["feat_name"], "feat_value")
