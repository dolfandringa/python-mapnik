...
if mason_build:
    ...
    for f in input_plugin_files:
        try:
            shutil.copyfile(
                f, os.path.join("mapnik", "lib", "mapnik", "input", os.path.basename(f))
            )
        except shutil.Error:
            pass
    ...
    for f in font_files:
        try:
            shutil.copyfile(
                f, os.path.join("mapnik", "lib", "mapnik", "fonts", os.path.basename(f))
            )
        except shutil.Error:
            pass
    f_paths.write(
...
extra_comp_args = list([arg for arg in extra_comp_args if arg != "-fvisibility=hidden"])

if os.environ.get("MAPNIK_USE_PROJ4", "false") == "true":
    extra_comp_args.append("-DMAPNIK_USE_PROJ4")
